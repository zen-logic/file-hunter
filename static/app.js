...
WS.on('scan_progress', (msg) => {
    const skippedSuffix = msg.filesSkipped ? `, ${msg.filesSkipped.toLocaleString()} deferred` : '';
    ...
    const skippedLog = msg.filesSkipped ? `, ${msg.filesSkipped.toLocaleString()} deferred` : '';
...
WS.on('scan_completed', async (msg) => {
    ...
    const skippedPart = msg.filesSkipped ? `, ${msg.filesSkipped.toLocaleString()} deferred` : '';
...
WS.on('consolidate_queue_drained', (msg) => {
    ...
    Toast.info(`${msg.jobsCompleted} pending consolidation job${msg.jobsCompleted === 1 ? '' : 's'} completed`);
});
